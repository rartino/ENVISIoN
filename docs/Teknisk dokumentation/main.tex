\documentclass[a4paper,12pt]{article}
\usepackage[swedish]{babel}
\usepackage[utf8]{inputenc}
\usepackage{graphicx}
\usepackage[title]{appendix}
\usepackage{amsmath}
\usepackage{epstopdf}
\usepackage{pdfpages}
\usepackage{xparse}
\usepackage{kantlipsum}
\usepackage{listings}
\usepackage{subcaption}
\usepackage{float}
\usepackage{titlesec}
\usepackage{subcaption}
\input{LIPS.tex}
\graphicspath{ {./images/} }
\lstset{literate=%
{å}{{\aa}}1
{ä}{{\"a}}1
{ö}{{\"o}}1
{Å}{{\AA}}1
{Ä}{{\"A}}1
{Ö}{{\"O}}1
}

\newcommand{\LIPSartaltermin}{2019/VT}
\newcommand{\LIPSkursnamn}{TFYA75}

\newcommand{\LIPSprojekttitel}{Elektronvisualisering}
\newcommand{\LIPSdokumenttyp}{Teknisk dokumentation}
\newcommand{\LIPSversion}{1.1}
\newcommand{\LIPSdatum}{\dagensdatum}

\newcommand{\LIPSgranskare}{}
\newcommand{\LIPSgranskatdatum}{}
\newcommand{\LIPSgodkannare}{}
\newcommand{\LIPSgodkantdatum}{}

\lhead{}
\chead{\textbf{\LIPSprojekttitel}}
\rhead{\textbf{\textsl{LiTH}}\\\textbf{\dagensdatum}}
\lfoot{\textbf{LIPS Teknisk dokumentation}}
\cfoot{\textbf{\thepage}}
\rfoot{\textbf{\LIPSkursnamn}}

\begin{document}
\pagenumbering{gobble}
\LIPStitelsida
\newpage
\pagenumbering{roman}
\input{Lips-front.tex}
\newpage
\tableofcontents{}
\newpage
\input{DokumentHistorik.tex}
\newpage
\input{Licens.tex}
\newpage
\pagenumbering{arabic}
\input{Inledning.tex}
\newpage
\input{Oversiktavsystemet.tex}
\newpage
\input{Parsersystemet.tex}
\newpage
\input{Visualiseringssystemet.tex}
\newpage
\input{GUI.tex}
\newpage
\input{Referenser.tex}
\newpage
\appendix
\input{AppendixHDF5.tex}
\newpage 
\input{AppendixGUI.tex}
\newpage 
\input{LicensAppendixtext.tex}
\newpage 
\input{Appendix_old_tekniskdok.tex}
\end{document}
